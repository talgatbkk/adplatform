...
<div class="text-center">
    <ul class="pagination justify-content-center">
        <c:if test="${requestScope.cur_page > 1}">
            <li><a class="btn btn-light" href="/advertisement/search?page=${requestScope.cur_page - 1}&location_item=${requestScope.location_selection}&category_item=${requestScope.category_selection}">${previousButtonLocal}</a></li>
        </c:if>
        <c:if test="${requestScope.total_ad_count / requestScope.page_ad_count > requestScope.cur_page}">
            <li><a class="btn btn-light" href="/advertisement/search?page=${requestScope.cur_page + 1}&location_item=${requestScope.location_selection}&category_item=${requestScope.category_selection}&search_input=${requestScope.prev_search_input}">${nextButtonLocal}</a></li>
        </c:if>
    </ul>
</div>
